...
func (h *VehicleHandler) GetVehicle(w http.ResponseWriter, r *http.Request) {
	...
	// Get vehicle using service
	vehicle, err := h.vehicleService.GetVehicleByID(r.Context(), vehicleID)
	if err != nil {
		log.Printf("Error fetching vehicle: %v", err)
		http.Error(w, "Failed to get vehicle: "+err.Error(), http.StatusNotFound)
		return
	}
